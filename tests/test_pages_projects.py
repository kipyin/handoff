...
def test_get_pending_changes_rename_and_archive(mock_projects):
    df = pd.DataFrame([
        {"__project_id": 1, "name": "Work Updated", "is_archived": False, "confirm_delete": False},
...
def test_execute_changes_calls_data_functions(monkeypatch):
    ...
    def mock_rename(pid, name): calls.append(("rename", pid, name))
    def mock_archive(pid): calls.append(("archive", pid))
    def mock_delete(pid): 
...
def test_execute_changes_handles_exceptions(monkeypatch):
    def mock_rename(pid, name): raise Exception("DB Error")
    monkeypatch.setattr("handoff.pages.projects.rename_project", mock_rename)
