...
- **Pre-existing test flake.** `test_init_db_raises_when_engine_creation_fails` in `tests/test_db.py` may fail in environments where `/nonexistent/dir/` triggers a `PermissionError` before the mock takes effect. This is a known environment-specific issue, not a code defect. Use `-k "not test_init_db_raises_when_engine_creation_fails"` to skip it if needed.
