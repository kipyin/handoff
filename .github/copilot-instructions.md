...
- **Linter/formatter:** Ruff (`uv run handoff-dev check --fix`). **Type checker:** Pyright (`uv run handoff-dev typecheck`).
- **Tests:** `uv run handoff-dev test` (pytest). Full CI: `uv run handoff-dev ci`.
