...
2. Bump version with `uv run handoff-dev bump YYYY.M.P` when user-visible changes ship.
...
5. Validate with `uv run handoff-dev ci`.
6. Run build commands needed for scope:
   - Full: `uv run handoff-dev build --full`
   - Patch: `uv run handoff-dev build --patch`
